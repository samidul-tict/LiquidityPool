...
describe("SpaceCoin ICO", function () {
  ...
  this.beforeEach(async function () {
    ...
    totalBalance = await spaceCoinICO
      .connect(owner)
      .balanceOf(spaceCoinICO.address);
  ...
  describe("toggle tax flag", function () {
    it("toggle tax flag, by owner", async function () {
      ...
      await expect(spaceCoinICO.connect(owner).toggleTaxFlag())
        .to.emit(spaceCoinICO, "ToggleTaxFlag")
        .withArgs(owner.address, !canDeductTax);
    });
    it("toggle tax flag, by someone other than owner", async function () {
      await expect(
        spaceCoinICO.connect(alice).toggleTaxFlag()
  ...
  describe("toggle ico status", function () {
    it("toggle ico status, by owner", async function () {
      ...
      await expect(spaceCoinICO.connect(owner).toggleICOStatus())
        .to.emit(spaceCoinICO, "ToggleICOStatus")
        .withArgs(owner.address, !icoStatus);
    });
    it("toggle ico status, by someone other than owner", async function () {
      await expect(
        spaceCoinICO.connect(alice).toggleICOStatus()
      ).to.be.revertedWith("NOT_OWNER");
    });
  });
  ...
  describe("change ico phase by user", function () {
    it("change ico phase from SEED => GENERAL => OPEN [forward direction], by owner", async function () {
      await expect(spaceCoinICO.connect(owner).changeICOStage(1))
        .to.emit(spaceCoinICO, "ChangeICOStage")
      ...
      await expect(
        spaceCoinICO.connect(owner).changeICOStage(3)
      ).to.be.revertedWith("NO_MORE_PHASE");
    });
    it("change ico phase, by someone other than owner", async function () {
      await expect(
        spaceCoinICO.connect(alice).changeICOStage(0)
      ).to.be.revertedWith("NOT_OWNER");
    });
  });
  ...
  describe("transfer token", function () {
    describe("transfer during SEED phase", function () {
      it("attempt transfer", async function () {
        await expect(
          spaceCoinICO.connect(owner).transfer(alice.address, 50)
    ...
    describe("transfer during OPEN phase, no TAX", function () {
      ...
      it("transfer by someone who has balance", async function () {
        await spaceCoinICO
          .connect(alice)
          .investInSPC({ value: ethers.utils.parseEther("1") });
        expect(
          await spaceCoinICO.connect(owner).balanceOf(alice.address)
        ).to.equal(ethers.utils.parseEther("5"));
        await spaceCoinICO.connect(alice).transfer(bob.address, 2);
        expect(
          await spaceCoinICO.connect(owner).balanceOf(alice.address)
        ).to.equal("4999999999999999998");
        expect(
          await spaceCoinICO.connect(owner).balanceOf(bob.address)
        ).to.equal("2");
      });
      it("transfer by someone who doesn't have balance", async function () {
        await expect(
          spaceCoinICO.connect(bob).transfer(alice.address, 50)
    ...
    describe("transfer during OPEN phase, with TAX deduction", function () {
      ...
      it("transfer by someone who has balance", async function () {
        ...
        await spaceCoinICO
          .connect(alice)
          .investInSPC({ value: ethers.utils.parseEther("200") });
        expect(
          await spaceCoinICO.connect(owner).balanceOf(alice.address)
        ...
        expect(
          await spaceCoinICO.connect(owner).balanceOf(treasury.address)
        ...
        expect(
          await spaceCoinICO.connect(owner).balanceOf(alice.address)
        ).to.equal("979999999999999999900");
        expect(
          await spaceCoinICO.connect(owner).balanceOf(bob.address)
        ).to.equal("98");
        expect(
          await spaceCoinICO.connect(owner).balanceOf(treasury.address)
        ).to.equal("20000000000000000002");
      });
      it("transfer by someone who doesn't have balance", async function () {
        await expect(
          spaceCoinICO.connect(bob).transfer(alice.address, 100)
        ).to.be.revertedWith("NOT_ENOUGH_TOKEN");
      });
    });
  });
        ...
        describe("invest by a whitelisted person", function () {
          it("via investInSPC() function", async function () {
            await expect(
              ...
              .withArgs(addrs[0].address, ethers.utils.parseEther("1500"), 0);
          });
          it("via receive() function", async function () {
            ...
            await expect(await addrs[1].sendTransaction(tx))
              .to.emit(spaceCoinICO, "InvestedInSPC")
              .withArgs(addrs[1].address, ethers.utils.parseEther("10"), 0);
          });
        ...
        describe("validate token balance during SEED phase", function () {
          it("invest and validate balance", async function () {
            await expect(
              ...
              .to.emit(spaceCoinICO, "InvestedInSPC")
              .withArgs(addrs[0].address, ethers.utils.parseEther("1"), 0);
            expect(await spaceCoinICO.balanceOf(addrs[0].address)).to.equal(
        ...
        describe("invest by not a whitelisted person", function () {
          it("via investInSPC() function", async function () {
            await expect(
              spaceCoinICO
                .connect(alice)
                .investInSPC({ value: ethers.utils.parseEther("10") })
          ...
          it("via receive() function", async function () {
            ...
            await expect(bob.sendTransaction(tx)).to.be.revertedWith(
              "NOT_WHITELISTED"
            );
          });
        });
      ...
      describe("invest during GENERAL phase", function () {
        ...
        it("invest by a whitelisted person", async function () {
          await expect(
            ...
            .to.emit(spaceCoinICO, "InvestedInSPC")
            .withArgs(addrs[0].address, ethers.utils.parseEther("10"), 0);
        });
        it("invest by not a whitelisted person", async function () {
          await expect(
            ...
            .to.emit(spaceCoinICO, "InvestedInSPC")
            .withArgs(alice.address, ethers.utils.parseEther("1000"), 0);
        });
        it("a person who reached INDIV_LIMIT_GENERAL", async function () {
          ...
          await expect(
            spaceCoinICO
              .connect(alice)
              .investInSPC({ value: ethers.utils.parseEther("100") })
        ...
        it("validate token balance during SEED phase", async function () {
          await expect(
            spaceCoinICO
              .connect(alice)
              .investInSPC({ value: ethers.utils.parseEther("1") })
            ...
            .to.emit(spaceCoinICO, "InvestedInSPC")
            .withArgs(alice.address, ethers.utils.parseEther("1"), 0);
          expect(await spaceCoinICO.balanceOf(alice.address)).to.equal(
            ethers.utils.parseEther("0")
          );
        });
      });
      describe("invest during OPEN phase", function () {
        ...
        it("invest by a whitelisted person without any restriction", async function () {
          await expect(
            spaceCoinICO
              .connect(addrs[10])
              .investInSPC({ value: ethers.utils.parseEther("2000") })
          )
            .to.emit(spaceCoinICO, "InvestedInSPC")
            .withArgs(
              addrs[10].address,
        ...
        it("invest after GOAL reached", async function () {
          await spaceCoinICO
            .connect(addrs[3])
            .investInSPC({ value: ethers.utils.parseEther("9999") });
          await spaceCoinICO
            .connect(addrs[4])
            .investInSPC({ value: ethers.utils.parseEther("9999") });
          await expect(
        ...
        it("validate token balance during OPEN phase", async function () {
          ...
          expect(await spaceCoinICO.balanceOf(addrs[6].address)).to.equal(
            ethers.utils.parseEther("5")
          );
        });
      });
  ...
  describe("withdraw fund from ICO", function () {
    describe("during OPEN phase", function () {
      this.beforeEach(async function () {
        // move to GENERAL phase
        await spaceCoinICO.connect(owner).changeICOStage(1);
      ...
      it("withdraw fund, by owner", async function () {
        expect(await spaceCoinICO.connect(owner).withdrawFund(treasury.address))
          .to.emit(spaceCoinICO, "FundWithdrawn")
          .withArgs(
            owner.address,
            treasury.address,
      ...
      it("withdraw fund, by someone other than owner", async function () {
        await expect(
          spaceCoinICO.connect(alice).withdrawFund(treasury.address)
        ).to.be.revertedWith("NOT_OWNER");
      });
    });
    describe("during SEED/ GENERAL phase", function () {
      this.beforeEach(async function () {
        await spaceCoinICO
          .connect(addrs[0])
          .investInSPC({ value: ethers.utils.parseEther("1500") });
      });
      it("withdraw fund, by owner during SEED phase", async function () {
        await expect(
          spaceCoinICO.connect(owner).withdrawFund(treasury.address)
        ).to.be.revertedWith("CANNOT_BE_WITHDRAWN");
      });
      it("withdraw fund, by owner during GENERAL phase", async function () {
        // move to GENERAL phase
        await spaceCoinICO.connect(owner).changeICOStage(1);
    ...
    describe("withdraw fund when total contribution = 0", function () {
      it("withdraw fund, by owner, SEED phase", async function () {
        await expect(
          spaceCoinICO.connect(owner).withdrawFund(treasury.address)
        ).to.be.revertedWith("CANNOT_BE_WITHDRAWN");
      });
      it("withdraw fund, by owner, GENERAL phase", async function () {
        // move to GENERAL phase
        await spaceCoinICO.connect(owner).changeICOStage(1);
      ...
      it("withdraw fund, by owner, OPEN phase", async function () {
        // move to GENERAL phase
        await spaceCoinICO.connect(owner).changeICOStage(1);
  ...
  describe("claim/ release spc", function () {
    describe("invest and claim during SEED phase", function () {
      it("via investInSPC() function", async function () {
        await expect(
          ...
          .to.emit(spaceCoinICO, "InvestedInSPC")
          .withArgs(addrs[7].address, ethers.utils.parseEther("1500"), 0);
        await expect(
          spaceCoinICO.connect(addrs[7]).claimToken(addrs[7].address)
    ...
    describe("invest and claim during GENERAL phase", function () {
      ...
      it("invest & claim by not a whitelisted person", async function () {
        await expect(
          ...
          .to.emit(spaceCoinICO, "InvestedInSPC")
          .withArgs(alice.address, ethers.utils.parseEther("200"), 0);
        await expect(
          spaceCoinICO.connect(alice).claimToken(alice.address)
        ).to.be.revertedWith("NOT_READY_TO_CLAIM");
      });
    });
    describe("invest and claim during OPEN phase", function () {
      ...
      it("claim token manually for all the investments made prior to OPEN phase", async function () {
        ...
        expect(
          await spaceCoinICO.connect(alice).balanceOf(alice.address)
      ...
      it("automatic release of token for all the investments made during OPEN phase", async function () {
        ...
        await spaceCoinICO
          ...
          .investInSPC({ value: ethers.utils.parseEther("100") });
        expect(
          await spaceCoinICO.connect(owner).balanceOf(addrs[0].address)
        ).to.equal(ethers.utils.parseEther("500"));
      });
    });
  });
